...
export function Modal({ title, description, onClose, isClosing }: ModalProps) {
  ...
  return (
    <div className='fixed top-0 left-0 w-full h-full bg-darkGray/50 flex items-center justify-center z-10 text-center'>
      <div className={`flex flex-col items-center  bg-white dark:bg-darkBlue p-4 rounded-md relative min-h-[200px] w-full mx-4 border-[1px]  border-lightGray sm:w-[500px] sm:p-6 sm:aspect-[2/1] ${isClosing ? 'animate-close' : 'animate-open'}`}>
